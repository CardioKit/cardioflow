...
public class MLFlwrClient: Client {
    ...
    public var logText = "Flower Client initialized."
    ...
    private func runMLTask(configuration: MLModelConfiguration, task: MLTask) -> MLResult {
        ...
        case .test:
            ...
            let eps = MLParameterKey.eps
            configuration.parameters = [epochs:1, eps:0.0001]
        ...
        let progressHandler: (MLUpdateContext) -> Void = { contextProgress in
            ...
            switch task {
            case .train:
                self.logText = "Epoch \(contextProgress.metrics[.epochIndex] as! Int + 1) finished with loss \(loss)"
                self.log.info("Epoch \(contextProgress.metrics[.epochIndex] as! Int + 1) finished with loss \(loss)")
            case .test:
                self.logText = "Evaluate finished with loss \(loss)"
