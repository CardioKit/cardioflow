...
class PolarSDKManager {
    ...
    private static let deviceId = "CA4DB523"
    
    var isBluetoothOn: Bool = false
    var isBroadcastListenOn: Bool = false
    var lengthSegmentSeconds: Int = 120 // 5*60
    
    var deviceConnectionState: DeviceConnectionState = DeviceConnectionState.disconnected(deviceId)
    var deviceSearch: DeviceSearch = DeviceSearch()
    
    var onlineStreamingFeature: OnlineStreamingFeature = OnlineStreamingFeature()
    var onlineStreamSettings: RecordingSettings? = nil
    
    var offlineRecordingFeature = OfflineRecordingFeature()
    var offlineRecordingSettings: RecordingSettings? = nil
    
    var offlineRecordingEntries: OfflineRecordingEntries = OfflineRecordingEntries()
    var offlineRecordingData: OfflineRecordingData = OfflineRecordingData()
    var deviceTimeSetupFeature: DeviceTimeSetupFeature = DeviceTimeSetupFeature()
    var sdkModeFeature: SdkModeFeature = SdkModeFeature()
    var h10RecordingFeature: H10RecordingFeature = H10RecordingFeature()
    var deviceInfoFeature: DeviceInfoFeature = DeviceInfoFeature()
    var batteryStatusFeature: BatteryStatusFeature = BatteryStatusFeature()
    var ledAnimationFeature: LedAnimationFeature = LedAnimationFeature()
    var generalMessage: Message? = nil
    ...
    func onlineStreamStart(feature: PolarDeviceDataType, settings: RecordingSettings? = nil, context: ModelContext) {
        ...
        switch feature {
        case .ecg:
            ecgStreamStart(settings: PolarSensorSetting(polarSensorSettings), context: context)
        case .acc:
    ...
    func generateRandomLetters() -> String {
        let letters = (0..<6).map { _ -> Character in
            let scalarValue = Int.random(in: 97...122)
    ...
    func ecgStreamStart(settings: PolarBleSdk.PolarSensorSetting, context: ModelContext) {
        if case .connected(let deviceId) = deviceConnectionState {
...
//                        }
                        ...
                        Task { @MainActor in
                            for item in data.samples {
                                self.voltage.append(Double(item.voltage))
                                if self.voltage.count == 130 * self.lengthSegmentSeconds {
                                    let recording = EcgRecording(signal: self.voltage, name: "Polar_" + self.generateRandomLetters())
                                    context.insert(recording)
                                    self.voltage.removeAll(keepingCapacity: false)
                                }
                            }
                        }
                    case .error(let err):
            ...
            onlineStreamingDisposables[.acc] = api.startAccStreaming(deviceId, settings: settings)
                ...
                .subscribe{ e in
                    ...
                    case .next(let data):
                        ...
                        for item in data.samples {
                            let force = sqrt(Double(item.x)*Double(item.x) + Double(item.y)*Double(item.y) + Double(item.z)*Double(item.z))
                            NSLog("ACC    x: \(item.x + 10000) y: \(item.y) z: \(item.z) timeStamp: \(item.timeStamp) Force: \(force)")
                        }
                    case .error(let err):
...
extension PolarSDKManager : PolarBleApiPowerStateObserver {
    func blePowerOn() {
        NSLog("BLE ON")
        Task { @MainActor in
            isBluetoothOn = true
...
extension PolarSDKManager : PolarBleApiObserver {
...
extension PolarSDKManager : PolarBleApiDeviceInfoObserver {
...
extension PolarSDKManager : PolarBleApiDeviceFeaturesObserver {
...
extension PolarSDKManager : PolarBleApiLogger {
